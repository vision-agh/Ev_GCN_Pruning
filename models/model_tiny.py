...
from utils.generate_outputs import conv_gen_out, conv_first_gen_out, graph_gen_out


class MyModelTiny(nn.Module):
    def __init__(self, cfg):
        super(MyModelTiny, self).__init__()
        self.cfg = cfg
    ...
    def forward(self, x: torch.Tensor,
        ...
        graph_gen_out(x, pos, edge_index, self.cfg, 'outputs/graph_out.txt')
        x = self.conv1(x, pos[:,:2], edge_index)
        conv_first_gen_out(x, pos, edge_index, self.cfg, 'outputs/conv1_out.txt')

        x, pos, edge_index, batch = self.pool1(x, pos, edge_index, batch)

        x = self.conv2(x, pos[:,:2], edge_index)
        conv_gen_out(x, pos, edge_index, self.cfg, 'outputs/conv2_out.txt')
        x = self.conv3(x, pos[:,:2], edge_index)
        conv_gen_out(x, pos, edge_index, self.cfg, 'outputs/conv3_out.txt')
