...
from utils.precompute_space_ncaltech import precompute_space_ncaltech
from utils.select_models import select_best_model
import pandas as pd
...
def main():
    ...
    baseline_model = LNRecognition.load_from_checkpoint(
        'checkpoints/ncaltech101_3.ckpt', cfg=cfg
    ).cuda()
    ...
    precomputed_space = precompute_space_ncaltech(cfg, depth_start=0, depth_end=100)
    ...
    for layer in precomputed_space:
        ...
        layer['m_space']       = np.array(layer['m_space'])[sorted_idx].tolist()
    ...
    c1_m,       c2_m,       c3_m,       c4_m,       c5_m       = [sorted_precomputed_space[i]['m_space']       for i in range(5)]
    ...
    def evaluate_at(idx):
        ...
        ms = (c1_m[idx[0]] + c2_m[idx[1]] + c3_m[idx[2]]
              + c4_m[idx[3]] + c5_m[idx[4]])
    ...
    # -───────────────────── get index of current model based on pruning and bits  ─────────────────────
    # find all index of best model that match with c{1-5}_pruning and c{1-5}_bits
    c1_idx = [i for i, x in enumerate(c1_pruning) if x == best_model['conv1_pruning'] and c1_bits[i] == best_model['conv1_bits']]
    c2_idx = [i for i, x in enumerate(c2_pruning) if x == best_model['conv2_pruning'] and c2_bits[i] == best_model['conv2_bits']]
    c3_idx = [i for i, x in enumerate(c3_pruning) if x == best_model['conv3_pruning'] and c3_bits[i] == best_model['conv3_bits']]
    c4_idx = [i for i, x in enumerate(c4_pruning) if x == best_model['conv4_pruning'] and c4_bits[i] == best_model['conv4_bits']]
    c5_idx = [i for i, x in enumerate(c5_pruning) if x == best_model['conv5_pruning'] and c5_bits[i] == best_model['conv5_bits']]

    current_idx = [c1_idx[0], c2_idx[0], c3_idx[0], c4_idx[0], c5_idx[0]]

    # ───────────────────── greedy search ─────────────────────
    # current_idx   = [0, 0, 0, 0, 0]
    best_acc, best_bram, best_m = evaluate_at(current_idx)
    print(f"Start → idx={current_idx}, acc={best_acc:.4f}, bram={best_bram}, mult={best_m}")
    ...
    while True:
        ...
        for i in range(1, 5):
            for idx_option in equal_bram_indices(bram_lists[i], current_idx[i]):
                ...
                acc, bram, m    = evaluate_at(new_idx)
                candidates.append((acc, bram, m, i, new_idx))

                print(f"    {layer_names[i]} → idx {idx_option} | acc={acc:.4f} | bram={bram} | mult={m}")
        ...
        acc_c, bram_c, m_c, layer_id, idx_c = max(candidates, key=lambda x: x[0])

        if acc_c <= best_acc - 0.05:
        ...
        best_bram   = bram_c
        best_m      = m_c
        print(f"↑ {layer_names[layer_id]} → idx {current_idx[layer_id]}  | acc={best_acc:.4f} | bram={best_bram} | mult={best_m}")
    ...
    print(f"conv1 {c1_pruning[current_idx[0]]} + {c1_bits[current_idx[0]]} bits | conv2 {c2_pruning[current_idx[1]]} + {c2_bits[current_idx[1]]} bits | conv3 {c3_pruning[current_idx[2]]} + {c3_bits[current_idx[2]]} bits | conv4 {c4_pruning[current_idx[3]]} + {c4_bits[current_idx[3]]} bits | conv5 {c5_pruning[current_idx[4]]} + {c5_bits[current_idx[4]]} bits")
    print(f"conv1 {c1_bram[current_idx[0]]} | conv2 {c2_bram[current_idx[1]]} | conv3 {c3_bram[current_idx[2]]} | conv4 {c4_bram[current_idx[3]]} | conv5 {c5_bram[current_idx[4]]}")
    print(f"BRAMs: {c1_bram[current_idx[0]] + c2_bram[current_idx[1]] + c3_bram[current_idx[2]] + c4_bram[current_idx[3]] + c5_bram[current_idx[4]]}")
    print(f"Multipliers: {c1_m[current_idx[0]] + c2_m[current_idx[1]] + c3_m[current_idx[2]] + c4_m[current_idx[3]] + c5_m[current_idx[4]]}")
    print(f"Accuracy: {best_acc:.4f} | BRAMs: {best_bram} | Multipliers: {best_m}")
